        ...
        <nav className="flex items-center space-x-3">
          {isAuthenticated ? (
            <>
              ...
              <Link to="/dashboard" className="p-2.5 rounded-2xl text-slate-400 hover:text-theme-primary hover:bg-theme-secondary transition-all active:scale-95">
              ...
              <div onClick={() => navigate('/profile')} className="w-10 h-10 rounded-2xl bg-slate-950 flex items-center justify-center text-white shadow-xl cursor-pointer active:scale-90 transition-all border border-white/10 group">
                <UserIcon className="w-5 h-5 group-hover:text-theme-primary transition-colors" />
              </div>
            </>
          ) : (
            <button 
              onClick={() => navigate('/login')}
              className="bg-slate-950 text-white text-[10px] font-black uppercase tracking-widest px-8 py-3 rounded-[1.2rem] shadow-xl hover:bg-theme-primary active:scale-95 transition-all"
            >
              Enter Portal
            </button>
          )}
